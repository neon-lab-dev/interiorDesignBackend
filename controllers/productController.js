...
const { deleteFile, uploadFile } = require("../utils/uploadFile");
...
exports.createProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  if (images && images.length > 0) {
    for (const image of images) {
      ...
      const mycloud = await uploadFile(fileUri.content, fileUri.fileName, "product-images");

      productImages.push({
        fileId: mycloud.fileId,
        name: mycloud.name,
        url: mycloud.url,
        thumbnailUrl: mycloud.thumbnailUrl,
      });
    }
  }
...
exports.getAllProducts = catchAsyncErrors(async (req, res, next) => {
  ...
  const apiFeature = new ApiFeatures(
    ...
    .search()
    .filter()
    .pagination(10);  // Pass the results per page, e.g., 10

  // Execute the query only once
  const products = await apiFeature.query;

  // Calculate filtered products count based on applied filters, if needed
  ...
  if (images && images.length > 0) {
    ...
    if (product.images && product.images.length > 0) {
      for (const existingImage of product.images) {
        await deleteFile(existingImage.fileId);
      }
    }
